...
- **Chosen**: 技術領域ごとに4スペックへ分割(B案)
  - backend-route-extractor: FastAPIのPython AST解析による抽出
...
- 拡張本体はTypeScriptで実装するVSCode拡張機能
...
- [ ] backend-route-extractor -- FastAPIのPythonコードをAST解析し、ルート定義(パス・method・OpenAPIスキーマ参照)とファイル/関数単位の呼び出しグラフを抽出する。Dependencies: none
