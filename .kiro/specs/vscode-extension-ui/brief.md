...
- 「導入するだけで動く」を満たすため、拡張マニフェスト(`package.json` の `engines.vscode`/`main`/`contributes`/`activationEvents`)を本spec実装時に必ず確立する。現状の `package.json` にはこれらが未設定で、VSCode拡張として認識されないため、アクティベーション設計と併せて定義する
