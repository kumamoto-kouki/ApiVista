 ...
 *
 * 視認性評価で「警告一覧とグラフが別々に表示されているだけで対応関係が分かりづらい」という
 * 指摘があったため、各`<li>`に`onTargetHover`を結線する。ホバー時に`warning.target`を渡し、
 * 離脱時に`null`を渡す(`main.ts`側がCytoscapeノードのハイライト切替に使う)。
 */
...
export function renderWarnings(
  ...
  for (const warning of warnings) {
    ...
    if (onTargetHover) {
      item.addEventListener("mouseenter", () => onTargetHover(warning.target));
      item.addEventListener("mouseleave", () => onTargetHover(null));
    }
